...
def parse_qml_classes(qml_path):
    ...
    for entry in root.findall(".//paletteEntry"):
        ...
        classes.append({"value": value, "label": label, "color": color})
    return classes
...
def create_raster_item():
    ...
    item.add_asset("data", pystac.Asset(href=raster_path, media_type=pystac.MediaType.COG, roles=["data"], title="LULC COG"))
    item.add_asset("thumbnail", pystac.Asset(href=raster_thumb, media_type=pystac.MediaType.PNG, roles=["thumbnail"], title="Raster Thumbnail"))
...
def create_vector_item():
    ...
    item.add_asset("data", pystac.Asset(href=vector_path, media_type=pystac.MediaType.GEOJSON, roles=["data"], title="SWB GeoJSON"))
    item.add_asset("thumbnail", pystac.Asset(href=vector_thumb, media_type=pystac.MediaType.PNG, roles=["thumbnail"], title="Vector Thumbnail"))
...
catalog = pystac.Catalog(id="gobindpur", title="Gobindpur STAC Catalog", description="Gobindpur catalog with LULC raster and SWB vector layers")
catalog.add_item(create_raster_item())
catalog.add_item(create_vector_item())
catalog.set_self_href(os.path.join(root_dir, "catalog.json"))
catalog.save(catalog_type=pystac.CatalogType.SELF_CONTAINED)

print(f"✅ STAC Catalog created at: {os.path.join(root_dir, 'catalog.json')}")
